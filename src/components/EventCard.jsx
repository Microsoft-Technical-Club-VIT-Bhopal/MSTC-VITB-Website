    ...
    >
        ...
        {event.image_path ? (
           <img 
             ...
             onError={(e) => {
               ...
               e.target.src = 'https://images.unsplash.com/photo-1633419461186-7d721f1de865?q=80&w=2070&auto=format&fit=crop'; // Abstract fallback
             }}
           />
        ) : (
          <div className="w-full h-full bg-gradient-to-br from-ms-blue to-ms-purple flex items-center justify-center">
            <span className="text-white font-display font-bold text-2xl opacity-50">MS Club</span>
          </div>
        )}
        <div className="absolute top-4 right-4 z-20">
          <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider backdrop-blur-md ${event.status === 'upcoming' ? 'bg-ms-green/20 text-ms-green border border-ms-green/30' : 'bg-slate-500/20 text-slate-500 border border-slate-500/30'}`}>
            {event.status}
          </span>
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          <button className="flex items-center gap-2 text-sm font-bold text-slate-900 dark:text-white hover:text-ms-blue dark:hover:text-ms-neon transition-colors group/btn">
            Details 
