...
export default function TeamCard({ member, rotate = "rotate-0" }) {
  ...
  return (
    <div className={`group relative w-full ${isSpecial ? 'aspect-[3/4] sm:aspect-video sm:col-span-2' : 'aspect-[3/4]'} rounded-[2rem] overflow-hidden cursor-pointer transition-all duration-300 border-4 ${isSpecial ? 'border-ms-blue dark:border-ms-blue' : 'border-slate-900 dark:border-white'} shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,1)] hover:shadow-none dark:hover:shadow-none hover:translate-x-[8px] hover:translate-y-[8px] hover:rotate-0 bg-white dark:bg-slate-900 ${rotate}`}>
      ...
      {/* Content */}
      <div className={`absolute ${isSpecial ? 'bottom-8 left-1/2 -translate-x-1/2 w-[85%] translate-y-0' : 'bottom-2 left-1/2 -translate-x-1/2 w-[90%] transform translate-y-2 group-hover:translate-y-0'} transition-transform duration-300 z-10`}>
        ...
        {/* Social Icons - Visible only on hover (except for Special which is always visible at bottom) */}
        {!isSpecial && (
        <div className="flex gap-3 mt-4 opacity-0 group-hover:opacity-100 transition-all duration-300 delay-100 justify-center">
          {member.linkedin_url && (
            <a 
              ...
              className="flex items-center justify-center p-3 bg-white border-2 border-black rounded-full text-black hover:bg-ms-blue hover:text-white hover:border-black transition-all duration-200 hover:scale-110 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[1px] hover:translate-y-[1px]"
          ...
          {member.instagram_url && (
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer"
              className="flex items-center justify-center p-3 bg-white border-2 border-black rounded-full text-black hover:bg-ms-neon hover:text-black hover:border-black transition-all duration-200 hover:scale-110 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[1px] hover:translate-y-[1px]"
              onClick={(e) => e.stopPropagation()}
            >
              <Instagram size={20} />
            </a>
      ...
      {/* Special Member Social Icons (Fixed at bottom) */}
      {isSpecial && member.linkedin_url && (
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer"
              className="flex items-center justify-center p-2 bg-white border-2 border-ms-blue rounded-full text-ms-blue hover:bg-ms-blue hover:text-white transition-all duration-200 hover:scale-110 shadow-[2px_2px_0px_0px_#00A4EF] hover:shadow-none hover:translate-x-[1px] hover:translate-y-[1px]"
              onClick={(e) => e.stopPropagation()}
            >
              <Linkedin size={20} />
            </a>
