...
export default function TeamCard({ member, rotate = "rotate-0" }) {
  return (
    <div className={`group relative w-full aspect-[3/4] rounded-[2rem] overflow-hidden cursor-pointer transition-all duration-300 border-4 border-slate-900 dark:border-white shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,1)] hover:shadow-none hover:translate-x-[8px] hover:translate-y-[8px] hover:rotate-0 bg-white dark:bg-slate-900 ${rotate}`}>
      {/* Background Image */}
      {member.image ? (
        <img 
          src={member.image} 
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-ms-blue/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
      ...
      <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-[90%] transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300 z-10">
        <div className="bg-white dark:bg-black border-2 border-slate-900 dark:border-white p-3 rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,1)] -rotate-1 group-hover:rotate-0 group-hover:shadow-none group-hover:translate-x-[4px] group-hover:translate-y-[4px] transition-all duration-300 text-center">
            <h3 className="text-lg font-black text-slate-900 dark:text-white font-display uppercase tracking-tight leading-none mb-1">
            {member.name}
            </h3>
            <p className="text-slate-600 dark:text-slate-300 font-bold text-[10px] uppercase tracking-wider">
            {member.role}
            </p>
        </div>
